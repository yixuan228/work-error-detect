...
def create_visualization_with_accidents(data, accident_dates, transfer_dates, sale_dates, treatment_dates, filename):
    """创建带有事故日期、转出日期、销售日期和治疗日期标注的可视化图表"""
    ...
    plt.title(f'{unit_name} - 饲喂数据可视化（标注事故/转出/销售/治疗日期）', fontsize=14, fontweight='bold', pad=20)
    ...
    # 保存图表
    output_filename = f'{unit_name}_可视化图表_标注事故转出销售治疗日期.png'
...
if os.path.exists(feeding_file):
    print(f"\n正在处理: {feeding_file}")
    data = read_and_process_file(feeding_file)
    if data is not None:
        print(f"成功读取 {len(data)} 条记录")
        create_visualization_with_accidents(data, accident_dates, transfer_dates, sale_dates, treatment_dates, feeding_file)
        print("\n图表已生成完成！")
    else:
        print("无法读取数据")
else:
