...
def read_and_process_by_pen(filepath):
    ...
    try:
        ...
        if head_count_col:
            agg_dict[head_count_col] = 'first'  # 每栏的猪只头数应该是一致的
        
        daily_data = df.groupby([pen_col, date_col]).agg(agg_dict).reset_index()
        
        # 重命名列
        if head_count_col:
            daily_data.columns = ['栏号', '日期', '每日采食总量', '每日喂水总量', '猪只头数']
        else:
        ...
        # 获取每栏的猪只头数（如果存在）
        pen_headcount = None
        if head_count_col and '猪只头数' in daily_data.columns:
            pen_headcount = daily_data.groupby('栏号')['猪只头数'].first().to_dict()
        
        return daily_data, pen_headcount
...
def create_visualization_by_pen(data, pen_number, filename, accident_dates, transfer_dates, sale_dates, treatment_dates, standards, pen_headcount, start_age=25, end_age=114):
    """为指定栏号创建带有事故/转出/销售/治疗日期标注和标准曲线的可视化图表"""
    ...
    # 获取该栏的猪只头数
    if pen_headcount and pen_number in pen_headcount:
        head_count = pen_headcount[pen_number]
    else:
        head_count = 40  # 默认值
        print(f"警告: 栏号 {pen_number} 未找到猪只头数，使用默认值 {head_count}")
    ...
    if standards:
        ...
        for date in dates:
            ...
            if daily_feed_per_head:
                # 计算每栏的标准采食量（头均日采食量 × 每栏猪只头数）
                standard_feed_per_pen = daily_feed_per_head * head_count
                # 计算标准饮水量（头均日采食量 × 3 × 每栏猪只头数）
                standard_water_per_pen = daily_feed_per_head * 3 * head_count
                
                standard_feed_list.append(standard_feed_per_pen)
                standard_water_list.append(standard_water_per_pen)
    ...
    plt.title(f'{unit_name} - 第{pen_number}栏 - 饲喂数据可视化（含标准曲线及事件标注）', fontsize=14, fontweight='bold', pad=20)
...
if os.path.exists(file):
    print(f"\n正在处理: {file}")
    data, pen_headcount = read_and_process_by_pen(file)
    ...
    if data is not None:
        ...
        if pen_headcount:
            ...
            for pen_num in target_pens:
                if pen_num in pen_headcount:
                    print(f"  第{pen_num}栏: {pen_headcount[pen_num]}头")
        ...
        for pen_num in target_pens:
            print(f"\n正在生成第 {pen_num} 栏的图表...")
            create_visualization_by_pen(data, pen_num, file, accident_dates, transfer_dates, sale_dates, 
                                       treatment_dates, standards, pen_headcount, start_age=25, end_age=114)
        
        print(f"\n所有图表已生成完成！共生成 {len(target_pens)} 个图表，保存在 '4栋2单元-含事件和标准' 文件夹中。")
    else:
        print("无法读取数据")
else:
    print(f"文件不存在: {file}")
